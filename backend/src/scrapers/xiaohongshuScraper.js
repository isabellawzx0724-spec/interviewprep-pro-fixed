...
function detectSignals(text = '') {
  ...
  return {
    ...
    contentDetected: /面经|面试|一面|二面|三面|终面|复盘|offer|笔试|自我介绍|追问/i.test(haystack)
  }
}
...
export async function scrapeXiaohongshu({ company, role, queries = [], limit = 24, cookie = '', headless = true, timeoutMs = 15000 }) {
  ...
  try {
    ...
    if (cookie) {
      ...
      console.info('[scrape/xiaohongshu] cookie injected into browser context')
    } else {
      console.warn('[scrape/xiaohongshu] cookie missing, running in fallback mode')
    }
